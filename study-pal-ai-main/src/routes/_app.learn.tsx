...
import { Loader2, RefreshCw, Sparkles, ChevronLeft, ChevronRight, MessageSquareCode } from "lucide-react";
...
import { LessonAnimation } from "@/components/remotion/LessonAnimation";
...
function LearnPage() {
  ...
  const cacheKey = prefs ? `lesson:${prefs.subject}:${prefs.topic}:${prefs.language}:${prefs.level}` : "";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-purple-900/60 to-[#0a1628] border border-purple-500/20 p-5 text-white shadow-[0_0_30px_rgba(139,92,246,0.15)]">
        <div className="text-[10px] font-bold uppercase tracking-wider text-purple-400">{SUBJECT_LABELS[prefs.subject]}</div>
        <h1 className="mt-1 text-2xl font-bold bg-gradient-to-r from-slate-100 to-slate-300 bg-clip-text text-transparent">{prefs.topic}</h1>
        <div className="mt-4 flex items-center gap-2">
          <Progress value={progressPct} className="h-2 bg-white/[0.06] [&>div]:bg-gradient-to-r [&>div]:from-purple-600 [&>div]:to-purple-400" />
      ...
      {error && (
        <div className="rounded-2xl border border-red-500/20 bg-red-500/[0.06] p-5 text-sm text-red-400">
          ...
          <Button size="sm" variant="outline" className="rounded-full border-red-500/30 bg-red-500/[0.05] hover:bg-red-500/[0.1] text-red-300" onClick={() => load(true)}>
      ...
      {lesson && !loading && (
        <div className="space-y-4 animate-[fadeSlideUp_0.4s_ease-out_forwards]">
          {/* Remotion Player under the header */}
          <LessonAnimation
            steps={lesson.animationSteps}
          ...
          <div className="relative overflow-hidden min-h-[220px]">
            ...
            {currentSlide === 0 && (
              <div className="rounded-2xl border border-purple-500/15 bg-white/[0.03] backdrop-blur-xl p-6 shadow-[0_4px_24px_rgba(139,92,246,0.1)] transition-all animate-[fadeSlideUp_0.3s_ease-out_forwards]">
                ...
                <div className="mt-6 flex justify-end">
                  <Button onClick={nextSlide} className="rounded-full bg-gradient-to-r from-purple-600 to-purple-400 text-white shadow-[0_0_20px_rgba(139,92,246,0.4)]">
            ...
            {currentSlide > 0 && currentSlide <= lesson.sections.length && (() => {
              ...
              return (
                <div className="rounded-2xl border border-purple-500/15 bg-white/[0.03] backdrop-blur-xl p-6 shadow-[0_4px_24px_rgba(139,92,246,0.1)] transition-all animate-[fadeSlideUp_0.3s_ease-out_forwards]">
                  ...
                  <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-slate-300">{section.body}</p>
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-purple-500/10 pt-4">
                    <button
                      onClick={() => openTutor(`Please explain "${section.heading}" from "${lesson.title}" in more detail with another everyday example.`)}
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={prevSlide} className="rounded-full border-purple-500/20 bg-white/[0.04] text-slate-200 hover:bg-white/[0.08]">
                        <ChevronLeft className="mr-1 h-4 w-4" /> Back
                      </Button>
                      <Button size="sm" onClick={nextSlide} className="rounded-full bg-gradient-to-r from-purple-600 to-purple-400 text-white shadow-[0_0_20px_rgba(139,92,246,0.4)]">
              ...
              <div className="space-y-4 transition-all animate-[fadeSlideUp_0.3s_ease-out_forwards]">
                ...
                <div className="rounded-2xl border border-purple-500/15 bg-white/[0.03] backdrop-blur-xl p-6 shadow-[0_4px_24px_rgba(139,92,246,0.1)]">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2.5">
                    <Button variant="outline" onClick={prevSlide} className="rounded-full border-purple-500/20 bg-white/[0.04] text-slate-200">
                      <ChevronLeft className="mr-1 h-4 w-4" /> Back
                    </Button>
                    <Button
                      onClick={finish}
                    ...
                    <Button variant="outline" onClick={() => load(true)} className="rounded-full border-purple-500/20 bg-white/[0.04] text-slate-200">
                  ...
                  {completed && (
                    <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] p-4 text-sm font-medium text-emerald-400 animate-pulse">
                      ✨ Awesome work! +20 XP has been added to your profile. Try a quiz or game next!
                    </div>
                  )}
